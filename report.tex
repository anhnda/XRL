...
\item \textbf{Limited expressiveness:} Struggle to represent conjunctive rules like ``Pickup $\leftarrow$ ($f_1 \land f_2 \land \neg f_3 \land f_4$)'' compactly
\end{itemize}
...
Raw observation --> CNN (frozen) --> Stage 1: SVD+ICA --> Normalize
                          ...
                          Stage 2B: Logic Training (SAE frozen)
                                         |
                          Sigmoid Bottleneck: sigmoid(α(z - β))
...
Notation: $f_i$ is feature $i$, $\neg f_i$ is its negation. ``x10'' indicates 10 redundant clauses were merged (learned same literals but different bias). Bias $b_i$ controls clause threshold.

...
def print_comparison(rules_results: dict, ppo_results: dict):
    ...
    print(f"\n  {'Action usage':22} {'Rules':>10} {'PPO':>10} {'Diff':>10}")
    print(f"  {'-'*52}")
    rules_counts = rules_results['action_counts']
    ppo_counts   = ppo_results['action_counts']
    rules_total  = max(sum(rules_counts), 1)
    ppo_total    = max(sum(ppo_counts), 1)
    for name, rc, pc in zip(ACTION_NAMES, rules_counts, ppo_counts):
        r_pct = 100.0 * rc / rules_total
        p_pct = 100.0 * pc / ppo_total
        print(f"  {name:<22} {r_pct:>9.1f}% {p_pct:>9.1f}% {r_pct - p_pct:>+9.1f}%")
